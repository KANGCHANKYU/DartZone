...
public class PT_01Dou222 : MonoBehaviour
{
    ...
    public Sprite[] s_TeamOn = new Sprite[3]; //자기팀의 차레일떄 on
    public Sprite[] s_TeamOff = new Sprite[3]; //자기팀의 차레가아닐떄 off 할 이미지
    ...
    void Update()
    {
        ...
        if (csSingleton.getInstance.g_bTarget && !isRead && !isSpace && !isWin && !isBust)
        {
            ...
            if (Round <= RoundStop && !isRead && !isBust)
            {
                ...
                /*----------------------CTeam------------------------------*/
                else if (Players == 0 && isCT && !isAT && !isBT && !isBust && !isRead)
                {
                ...
                else if (Players == 1 && isCT && !isAT && !isBT && !isBust && !isRead)
                {
                    if (Counting == 0 && isCT && !isAT && !isBT && !isBust && !isRead)
                    {
                        DartCountC2++;
        ...
        if (Input.GetKeyDown(KeyCode.Space) && isBust && csgmg.isAni == false)
        {
            ...
            if (Teams == 2 && Players == 0) //팀 마이너스 및 플레이어 플러스
            {
                a_source.PlayOneShot(ac_shoot2, 1.0f);
                for (int i = 0; i < g_Dart.Length; i++)
                    g_Dart[i].GetComponent<Image>().sprite = s_OffDart;
