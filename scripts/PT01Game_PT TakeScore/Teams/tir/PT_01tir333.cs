...
public class PT_01tir333 : MonoBehaviour
{
    public Text t_RoundText; //라운드 텍스트
    public Text[] t_PPDText = new Text[9]; //각각 ppd 
    ...
    public GameObject[] g_RoundScore = new GameObject[9]; //각 플레이어의 라운드 스코어를 가지고있는 부모
    public GameObject[] g_PPDScore = new GameObject[9]; //각 플레이어의 PPD 스코어를 가지고있는 부모
    ...
    public string[] PlayerName = new string[9]; //각 플레이어의 이름을 저장해줄변수
    ...
    void Update()
    {
        ...
        if (Round == RoundStop && Counting == 3 && isCT && !isWin)
            GameEndRead();
    }
